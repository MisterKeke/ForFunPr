...
internal sealed class BookmarkRepositoryTestHarness : IAsyncDisposable
{
    ...
    public BookmarkService Service { get; }

    public static async Task<BookmarkRepositoryTestHarness> CreateAsync(
        CancellationToken cancellationToken)
    {
        ...
        var repository = new BookmarkRepository(connectionFactory, writeCoordinator);
        var service = new BookmarkService(repository, new WeakReferenceMessenger());
        return new BookmarkRepositoryTestHarness(paths, initializer, writeCoordinator, service);
    }
